(function () {
  ...
  const parentMenuLinks = document.querySelectorAll(
    ".linksmm-primary-nav .menu-item-has-children > a",
  );

  function closeSubmenus(scope) {
    const container = scope || document;
    const openItems = Array.from(
      container.querySelectorAll(".menu-item-has-children.is-submenu-open"),
    );

    if (
      container.matches &&
      container.matches(".menu-item-has-children.is-submenu-open")
    ) {
  ...
  parentMenuLinks.forEach(function (link) {
    link.addEventListener("click", function (event) {
      ...
      parentList
        .querySelectorAll(":scope > .menu-item-has-children.is-submenu-open")
  ...
  document
    .querySelectorAll("[data-quantity-minus], [data-quantity-plus]")
    .forEach(function (button) {
      button.addEventListener("click", function () {
        const quantity = button.closest(".linksmm-quantity");
        ...
        input.value = Math.min(
          maximum,
          Math.max(minimum, Number(input.value || minimum) + direction),
        );
        input.dispatchEvent(
          new Event("change", {
            bubbles: true,
  ...
  document.querySelectorAll(".linksmm-testimonial-slider").forEach(function (slider) {
    const swiperElement = slider.querySelector(".linksmm-testimonial-swiper");
    ...
    new Swiper(swiperElement, {
      ...
      pagination: {
        el: slider.querySelector(".linksmm-testimonial-dots"),
        clickable: true,
      },
      navigation: {
        nextEl: slider.querySelector(".linksmm-testimonial-next"),
        prevEl: slider.querySelector(".linksmm-testimonial-prev"),
      },
      breakpoints: {
        992: {
          slidesPerView: 3,
  ...
  document
    .querySelectorAll(".linksmm-product-gallery")
    .forEach(function (gallery) {
      const mainImage = gallery.querySelector("[data-product-main-image]");
      const mainLink = gallery.querySelector("[data-product-gallery-link]");
      const buttons = gallery.querySelectorAll("[data-gallery-image]");

      buttons.forEach(function (button) {
        button.addEventListener("click", function () {
          const imageSource = button.dataset.galleryImage;

          if (mainImage && imageSource) {
            mainImage.src = imageSource;
            mainImage.alt = button.dataset.galleryAlt || "Service preview";
            mainImage.setAttribute("data-large_image", imageSource);
          }

          if (mainLink && imageSource) mainLink.href = imageSource;
  ...
  document
    ...
    .forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        const item = button.closest(".linksmm-accordion-item");
        const panel = document.getElementById(
        ...
        accordion
          .querySelectorAll(".linksmm-accordion-item")
